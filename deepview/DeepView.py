...
class DeepView:
	...
	def _init_plots(self):
		...
		# plot wrong predictions of model as circles around samples
		# for c in range(self.n_classes):
		# 	color = self.cmap(c/(self.n_classes-1))
		...
		# visualize wrong predictions of model as a coloured cross in the sample
		for c in range(self.n_classes):
			color = self.cmap(c/(self.n_classes-1))
			plot = self.ax.plot([], [], '+', color=color, mew=0.5, 
		    zorder=3)
			self.sample_plots.append(plot[0])

		# draw a red circle around modified data points
		color = 'red'
		plot = self.ax.plot([], [], 'o', markeredgecolor=color, 
			fillstyle='none', ms=12, mew=1, zorder=1)
		self.sample_plots.append(plot[0])
	...
	def queue_samples(self, samples, labels, preds, modified):
		'''
		...
		self.modified = np.concatenate((modified, self.modified))[:self.max_samples]

	def add_samples(self, samples, labels, modified):
		'''
		...
		modified : array-like
			List with information if samples have been modified [n_samples, 1]
		'''
		# get predictions for the new samples
		Y_probs = self._predict_batches(samples)
		Y_preds = Y_probs.argmax(axis=1)
		# add new values to the DeepView lists
		self.queue_samples(samples, labels, Y_preds, modified)
	...
	def show(self):
		...
		# visualize modified examples:
		data = self.embedded[self.modified==1]
		self.sample_plots[self.n_classes+self.n_classes].set_data(data.transpose())
